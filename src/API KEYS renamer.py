# TMDb API Setup (Replace with your own API Key)
TMDB_API_KEY = "865235ff7997022ab64b1c540811d004"
TVDB_API_KEY = "5c7b8ce5-ab0b-469e-ade4-b088718a261b"
TMDB_BASE_URL = "https://api.themoviedb.org/3"
TVDB_BASE_URL = "https://api.thetvdb.com"

curl -X POST https://api4.thetvdb.com/v4/login -H "Content-Type: application/json" -d "{\"apikey\":\"5c7b8ce5-ab0b-469e-ade4-b088718a261b\"}"

SIMKL API Keys:

Client ID:  9cbde7d60627f7067944b69559bf1ad907b4be8fa4008a4a2e8458b12204ba36
Client Secret: 16b17ac3d8d684385e6305093f416d6b3befdd65607b6b46ce9e2ef1a7bd1b9a

OMDb API Key:

1dcc84eb